...
const ceeCourses = require('../../model/ceeCourse')
...
router.get('/:id', async (req,res)=>{
    try {
        ...
        const Course = await ceeCourses.find({semester:parameter}).exec();
        res.send(Course)
    } catch (error) {
        res.json(error)
    }
...
router.post("/:id", async(req, res) => {
    ...
    try {
      const alreadyExists = await ceeCourses.findOne({ coursecode: req.body.coursecode }).exec();
  if(!alreadyExists)
  {
    ...
    course
      ...
      const ceeCourse = new ceeCourses({
        title: ToSave.title,
        mail: req.body.mail,
